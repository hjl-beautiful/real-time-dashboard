...
for i, (col, title, col_name, color, fill_color) in enumerate(chart_configs):
    with col:
        st.markdown(f"<p style='color:#94a3b8; font-size:13px; font-weight:600; margin-bottom:8px;'>{title}</p>", unsafe_allow_html=True)
        fig = go.Figure()
        ...
        fig.update_layout(
            ...
            xaxis=dict(showgrid=False, tickfont=dict(color="#64748b", size=10), tickangle=45, nticks=8),
            yaxis=dict(showgrid=True, gridcolor="rgba(51,65,85,0.5)", tickfont=dict(color="#64748b", size=10), zeroline=False),
...
with col1:
    st.markdown("<p style='color:#94a3b8; font-size:13px; font-weight:600; margin-bottom:8px;'>全国用户分布热力图</p>", unsafe_allow_html=True)
    ...
    fig_map.add_trace(go.Bar(
        ...
        text=[f"{v}" for v in reversed(map_values)], textposition="outside", textfont=dict(color="#94a3b8", size=11),
...
with col3:
    ...
    fig_top.add_trace(go.Bar(
        ...
        text=[f"{v}" for v in reversed(top_df["销量"].tolist())], textposition="outside", textfont=dict(color="#94a3b8", size=10),
